...
class CoinPriceViewCli:
    ...
    def _convert_pricedata_to_df(self, pricedata: list[CoinPriceData]) -> pd.DataFrame:
        ...
        df.sort_values(
            by=["coin.name", "curr"], key=lambda col: col.str.lower(), inplace=True
        )
        return df
    ...
    def show_help(self) -> None:
        ...
        print(
            "(H)ist date - get historical prices via market chart, when no date, the default date is taken"
        )
        print(
            "H2 date or hist2 date - get historical prices (only CoinGecko), when no date, the default date is taken"
        )
        print("DB - use coins defined in database (default)")
        print("(C)oin coin_id - use the coin id (can be multiple)")
        print(
            "Curr currency_id - use the currency id (can be multiple), defaults to btc,eth,eur,usd"
        )
    ...
    def price_current(self, control: PriceController) -> None:
        ...
        self.print_coinpricedata(f"* Current price of coins", self.price_data)
    ...
    def price_hist(self, control: PriceController, date: str) -> None:
        ...
        self.print_coinpricedata("* History price of coins", self.price_data)
    ...
    def price_hist_marketchart(self, control: PriceController, date: str) -> None:
        ...
        self.price_data = control.get_price_hist_marketchart(self.last_date)
        self.print_coinpricedata(
            "* History price of coins via marketchart", self.price_data
        )
    ...
    def str_to_coindata(self, data: str) -> list[CoinData]:
        """Make a list of string of values"""
        coins = re.split("[;,]", data)
        coin_data = [CoinData(siteid=i, chain=self.chain, symbol=i) for i in coins]
        return coin_data
    ...
    def ui_root(self, control: PriceController, date: str) -> None:
        ...
        while True:
            ...
            match cmd:
                ...
                case Command(
                    command="curr" | "currency" | "currencies", arguments=[rest]
                ):
                ...
                case Command(command="xls"):
                    self.write_to_file(control, self.price_data, OutputFileType.XLSX)
                case Command(command="csv"):
                    self.write_to_file(control, self.price_data, OutputFileType.CSV)
                case Command(
                    command="a" | "auto" | "all",
                    arguments=["xls" | "csv" | "both", *rest],
                ):
                    pass
                case _:
